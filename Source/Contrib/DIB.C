...
{
   return (LPSTR)( (INT)lpbi + (INT)*(LPDWORD)lpbi + (INT)PaletteSize( lpbi ) );
}
/* End FindDIBits() *********************************************************/
...
LPBYTE ConvertDIBFormat( LPBITMAPINFO lpSrcDIB, UINT nWidth, UINT nHeight, UINT nbpp, BOOL bStretch )
{
    ...
    hDC = GetDC( NULL );
    hTargetBitmap = CreateDIBSection( hDC, lpbmi, DIB_RGB_COLORS, (PVOID*)&lpTargetBits, NULL, 0 );
    hSourceBitmap = CreateDIBSection( hDC, lpSrcDIB, DIB_RGB_COLORS, (PVOID*)&lpSourceBits, NULL, 0 );
    ...
    else
    {
        ...
        if( bStretch )
        {
            ...
            lpbmi->bmiHeader.biHeight, hSourceDC, 0, 0, 
