...
BOOL SaveIconToFile(HICON hIcon, LPTSTR szFileName, int nBit) 
{ 
	BOOL ret=FALSE; 
	PVOID MonoBits; 
	PVOID ColorBits; 
	...
	if(IconInfo.fIcon)
	{
			...
			)
		{
			...
			tIH.wType=IconInfo.fIcon;
			tIH.Count = 1;
			ret = WriteFile(hFile, &tIH, sizeof(TIconHeader), &dwWrite, NULL); 
			tIR.Width = (BYTE)ColorInfo->bmiHeader.biWidth; 
			tIR.Height = (BYTE)ColorInfo->bmiHeader.biHeight; 
...
HICON GetIconUsedForFile(LPTSTR szFileName, BOOL IsSmallIcon)
{
	SHFILEINFO sfi={0};
	
	SHGetFileInfo(szFileName, 
		PathIsDirectory(szFileName)?FILE_ATTRIBUTE_DIRECTORY:FILE_ATTRIBUTE_NORMAL, 
		&sfi, sizeof(SHFILEINFO), 
		SHGFI_ICON|SHGFI_USEFILEATTRIBUTES|
		(IsSmallIcon?SHGFI_SMALLICON:SHGFI_LARGEICON));
